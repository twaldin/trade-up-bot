...
- **Test database:** the daily fire requires `TEST_DATABASE_URL` from the host-only 0600 `~/.config/trade-up-bot/autoresearch.env`, pointing to the dedicated localhost-only `tradeupbot_test` role/database. Missing/unreadable configuration, a non-test database name, wrong role, or failed connection fails the fire; integration gates are never skipped or redirected to production.
- **Privilege boundary:** all services use `NoNewPrivileges=true`, no capabilities, private temporary/device views, strict read-only system/home mounts, and narrowly listed write paths. Docker sockets are inaccessible. The daily agent needs only its checkout/state/auth caches, the unprivileged test DB, ordinary GitHub/OMP network access, read-only Deckbox SSH configuration, and SSH deploy access to the documented VPS; autonomous deploy authority does not include Deckbox root or `sudo`.
