...
for (const device of ["desktop", "mobile"]) {
  ...
  take(`${p}-tradeup`, device, null, async ({ page, rec, ptr, t }) => {
    ...
    const signin = page.getByText("Sign in to verify, claim, and purchase listings").first();
    ...
    await shot(page, t, page.getByText("Sign in to verify, claim, and purchase listings").first().locator("xpath=.."), "signin", 6);
  });
